...
parser.add_argument("-np", "--num-processes", help="Number of processors to use",
                    type=int, default=2)
parser.add_argument(
    "-s", "--split", help="Split size, in Mb", type=int, default=32)
...
def do_part_download(args):
    ...
    logger.debug("Reading HTTP stream in %dM chunks" %
    ...
    try:
        ...
        os.close(fd)
        s = s / 1024 / 1024.
        logger.debug("Downloaded %0.2fM in %0.2fs at %0.2fMBps" %
                     (s, t2, s / t2))
...
def main(src, dest, num_processes=2, split=32, force=False, verbose=False, quiet=False, secure=True, max_tries=5):
    ...
    if size < 1024 * 1024:
        ...
        try:
            ...
            pool.map_async(do_part_download, arg_iterator(
                num_parts)).get(9999999)
            t2 = time.time() - t1
